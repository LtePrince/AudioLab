...
if __name__ == "__main__":
    file_path = "/home/adolph/Share_Document/code2/rhythm_generation/AudioLab/data/audio/Eltaw.ogg"
    npy_path  = "/home/adolph/Share_Document/code2/rhythm_generation/AudioLab/data/audio/Eltaw.npy"

    # --- CPU Processor ---
    cpu_processor = AudioCPUprocessor()

    # process and visualize
    log_mel_spec_cpu = cpu_processor.process(file_path, debug_mode=False)
    print(f"[CPU] log-mel spectrogram : {log_mel_spec_cpu.shape}")   # (n_mels, Frames)

    # save to npy
    cpu_processor.save(log_mel_spec_cpu, npy_path)
    print(f"[CPU] saved to            : {npy_path}")

    # --- GPU Processor ---
    gpu_processor = AudioGPUprocessor()  # auto-selects cuda if available

    # load raw waveform and run forward
    waveform = gpu_processor.load_from_path(file_path)
    print(f"[GPU] waveform            : {waveform.shape}")            # (1, Channel, Time)
    out = gpu_processor.forward(waveform)
    print(f"[GPU] forward output      : {out.shape}")                 # (1, n_mels, Frames)

    # load pre-saved npy from CPU processor
    mel_spec = gpu_processor.load_mel_spec(npy_path)
    print(f"[GPU] mel spec from npy   : {mel_spec.shape}")            # (1, n_mels, Frames)

    # # --- Batch Processing ---
    # batch_txt    = "/home/adolph/Share_Document/code2/rhythm_generation/AudioLab/data/data.txt"
    # npy_out_dir  = "/home/adolph/Share_Document/code2/rhythm_generation/AudioLab/data/npy"
    # npy_list     = "/home/adolph/Share_Document/code2/rhythm_generation/AudioLab/data/npy_list.txt"

    # # CPU: batch process then save
    # specs = cpu_processor.process_batch(batch_txt)
    # print(f"[CPU] batch processed     : {len(specs)} file(s), shape: {specs[0].shape}")
    # cpu_processor.save_batch(specs, batch_txt, npy_out_dir, npy_list)
    # print(f"[CPU] batch saved to      : {npy_out_dir}, list: {npy_list}")

    # # GPU: batch load from npy list
    # mel_spec_batch = gpu_processor.load_mel_spec_batch(npy_list)
    # print(f"[GPU] mel spec batch      : {mel_spec_batch.shape}")      # (Batch, n_mels, Frames)