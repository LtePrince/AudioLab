...
if __name__ == "__main__":
    ...
    try:
        ...
        print("  [FAIL] mutation was not blocked")
    except AttributeError:
        print("  [OK]   mutation correctly blocked")

    # --- 2. Flatten -> PhigrosFlatChart ---
    # frame_ms = 512/22050/4 * 8 * 1000 approx 46.44 ms
    ...
    # --- 3. chart_to_array shortcut ---
    arr2, vf2 = conv.chart_to_array(chart)
    assert arr2.shape == flat_chart.note_array.shape
    assert np.allclose(arr2, flat_chart.note_array), "chart_to_array mismatch"
    print("\n=== chart_to_array ===")
    print(f"  [OK] output matches flatten()  shape={arr2.shape}")

    # --- 4. array_to_notes round-trip ---
    recovered = conv.array_to_notes(flat_chart.note_array, bpm=chart.bpm)
    print("\n=== array_to_notes (round-trip) ===")
    print(f"  decoded notes  : {len(recovered)}")
    print(f"  first decoded  : {recovered[0]}")

    # # --- 5. save_phigros_file (P1-6) ---
    # conv.save_phigros_file(
    #     flat_chart.note_array,
    #     bpm=chart.bpm,
    #     output_path=_OUT,
    #     offset=chart.offset,
    # )
    # saved = parse_phigros_file(_OUT)
    # print("\n=== save_phigros_file ===")
    # print(repr(saved))
    # print(f"  notes in saved file : {saved.total_note_count}")
    # print(f"  judge_lines         : {len(saved.judge_lines)}")
    # print(f"  first saved note    : {saved.judge_lines[0]['notesAbove'][0]}")
